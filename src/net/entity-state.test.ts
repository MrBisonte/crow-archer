...
describe('player state packing', () => {
  ...
  it('carries the flags back', () => {
    expect(roundTrip({ dead: true, shielded: false, aim: 0, swing: 0 })).toMatchObject({
      dead: true,
      shielded: false,
    });
    expect(roundTrip({ dead: false, shielded: true, aim: 0, swing: 0 })).toMatchObject({
      dead: false,
      shielded: true,
    });
  });
  ...
  it('carries both flags at once, since a shielded body can also go down', () => {
    expect(roundTrip({ dead: true, shielded: true, aim: 0, swing: 0 })).toMatchObject({
      dead: true,
      shielded: true,
    });
  });
  ...
  it('keeps the aim angle within the quantisation step', () => {
    for (const aim of [0, 0.5, 1, 2, 3, 4, 5, 6.2]) {
      const back = roundTrip({ dead: false, shielded: false, aim, swing: 0 });
  ...
  it('normalises a negative angle instead of letting the sign reach the flags', () => {
    const back = roundTrip({ dead: false, shielded: false, aim: -Math.PI / 2, swing: 0 });
  ...
  it('normalises an angle past a full turn', () => {
    const back = roundTrip({ dead: false, shielded: false, aim: TAU + 1, swing: 0 });
  ...
  it('carries swing progress back to within a sixteenth', () => {
    for (const swing of [0, 0.25, 0.5, 0.75, 0.99]) {
      const back = roundTrip({ dead: false, shielded: false, aim: 0, swing });
  ...
  it('reports not swinging as exactly zero, so a rest pose is never a swing', () => {
    expect(roundTrip({ dead: false, shielded: false, aim: 1, swing: 0 }).swing).toBe(0);
  });

  it('keeps every field independent', () => {
    const back = roundTrip({ dead: true, shielded: true, aim: 3, swing: 0.5 });
  ...
  it('packs to a non-negative integer, which is what the wire encodes', () => {
    const packed = packPlayerState({ dead: true, shielded: true, aim: 5.9, swing: 0.9 });
