...
const FORESHADOW = (() => {
  const _MILESTONES = [
    ...
    { at: 8, text: '— the Murder King awakens —',     tint: 0.28, screech: true },
...
let shake = { mag: 0, dur: 0, maxDur: 0 };

function triggerShake(mag, ms) {
  if (mag > shake.mag) { shake.mag = mag; shake.dur = ms/1000; shake.maxDur = ms/1000; }
}
function updateShake(dt) {
  if (shake.dur > 0) { shake.dur = Math.max(0, shake.dur - dt); if (shake.dur === 0) shake.mag = 0; }
}
function shakeOffset(t) {
  if (shake.dur <= 0) return { x: 0, y: 0 };
  const d = shake.dur / shake.maxDur;
  return { x: Math.sin(t*90)*shake.mag*d, y: Math.cos(t*75)*shake.mag*d };
}

// Static tile art is painted once into an offscreen layer and blitted per
// frame; only water, tree flicker, and ash embers draw live on top.
// Painters draw the static part of one tile at layer-local (x, y).
// Adding a tile type means adding a painter here, not editing a loop.
const TILE_PAINTERS = {
  [TILE.EMPTY](g, x, y, seed) {
    const ts = CONFIG.tileSize;
    g.fillStyle = '#1a2a1a'; g.fillRect(x, y, ts, ts);
    // Subtle ground texture: deterministic micro-spots per tile
  ...
  [TILE.WATER](g, x, y) {
    ...
    g.fillStyle = '#1a4a8a'; g.fillRect(x, y, CONFIG.tileSize, CONFIG.tileSize);
  },
  [TILE.TREE](g, x, y) {
    const ts = CONFIG.tileSize;
    g.fillStyle = '#1a2a1a'; g.fillRect(x, y, ts, ts);
  ...
  [TILE.HUT](g, x, y, seed, hutAbove, hutLeft) {
    const ts = CONFIG.tileSize;
    // Ground beneath
    g.fillStyle = '#1a2a1a'; g.fillRect(x, y, ts, ts);
    // Outer wall — clay / stone
    g.fillStyle = '#b89060';
    ...
    // Roof: only on top row of hut cluster — dark terracotta peak
    if (!hutAbove) {
      g.fillStyle = '#7a3010';
      g.beginPath();
    ...
    // Door: only on bottom-left tile of cluster, centred
    if (!hutLeft && hutAbove) {
      g.fillStyle = '#3a1f08';
...
// TileMap event invalidates; the per-frame cost is a single drawImage.
class StaticTileLayer {
  constructor(map) {
    this.map = map;
    this.canvas = document.createElement('canvas');
    this.canvas.width  = CONFIG.cols * CONFIG.tileSize;
    this.canvas.height = CONFIG.rows * CONFIG.tileSize;
    ...
    for (let dr = -1; dr <= 1; dr++)
      for (let dc = -1; dc <= 1; dc++) {
        ...
        if (nr >= 0 && nr < this.map.rows && nc >= 0 && nc < this.map.cols)
          this.paintTile(nr, nc);
      }
  }
  paintTile(r, c) {
    const ts = CONFIG.tileSize, x = c * ts, y = r * ts;
    this.g.clearRect(x, y, ts, ts);
    const painter = TILE_PAINTERS[this.map.get(r, c)];
    if (!painter) return;
    const hutAbove = r > 0 && this.map.get(r-1, c) === TILE.HUT;
    const hutLeft  = c > 0 && this.map.get(r, c-1) === TILE.HUT;
    painter(this.g, x, y, r*97 + c*31, hutAbove, hutLeft);
  }
  draw(target) { target.drawImage(this.canvas, 0, CONFIG.hudHeight); }
}
...
class AnimatedTileOverlay {
  constructor(map) {
    this.map = map;
    this.water = []; this.trees = []; this.ash = [];
    map.onReset(() => this.rebuild());
    map.onChange(() => this.rebuild());
  }
  rebuild() {
    const ts = CONFIG.tileSize, HH = CONFIG.hudHeight;
    this.water.length = 0; this.trees.length = 0; this.ash.length = 0;
    for (let r = 0; r < this.map.rows; r++)
  ...
  draw(target, t, phase) {
    ...
    for (const w of this.water) target.fillRect(w.x, w.y, CONFIG.tileSize, CONFIG.tileSize);
    ...
    for (const tr of this.trees) {
      ...
      target.beginPath(); target.arc(tr.x+14, tr.y+10, 5, 0, Math.PI*2); target.fill();
    }
...
const tileLayer   = new StaticTileLayer(tileMap);
const tileOverlay = new AnimatedTileOverlay(tileMap);

// The vignette gradient never changes for a fixed-size canvas, so bake it once.
// Full-size canvas with only the below-HUD region filled, so geometry and blit
// offset match the original inline gradient exactly.
const vignetteCanvas = (() => {
  const cv = document.createElement('canvas');
  cv.width = CONFIG.canvasW; cv.height = CONFIG.canvasH;
  const g = cv.getContext('2d');
  const vg = g.createRadialGradient(
    CONFIG.canvasW/2, CONFIG.canvasH/2, CONFIG.canvasH*0.35,
    CONFIG.canvasW/2, CONFIG.canvasH/2, CONFIG.canvasH*0.80);
  vg.addColorStop(0, 'rgba(0,0,0,0)'); vg.addColorStop(1, 'rgba(0,0,0,0.52)');
  g.fillStyle = vg; g.fillRect(0, CONFIG.hudHeight, CONFIG.canvasW, CONFIG.canvasH - CONFIG.hudHeight);
  return cv;
})();
...
// runtime cost is a drawImage instead of a shadowBlur raster pass per draw.
class StampCache {
  ...
  get(key, w, h, painter) {
    ...
    if (!s) {
      ...
      s.width = Math.ceil(w); s.height = Math.ceil(h);
      painter(s.getContext('2d'), s.width, s.height);
      this.map.set(key, s);
    }
    return s;
  }
...
// Radius quantized to 0.25 px and blur to 1 px so the cache stays bounded
// while shrinking particles reuse stamps.
function glowDotStamp(color, r, blur) {
  ...
  return stamps.get(`dot|${color}|${rq}|${bq}`, size, size, (g, w, h) => {
    g.shadowColor = color; g.shadowBlur = bq;
    g.fillStyle = color;
    g.beginPath(); g.arc(w / 2, h / 2, rq, 0, Math.PI * 2); g.fill();
  });
}

function glowRectStamp(color, rw, rh, blur) {
  const bq = Math.max(1, Math.round(blur)), pad = bq * 2 + 2;
