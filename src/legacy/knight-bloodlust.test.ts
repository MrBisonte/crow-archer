...
import { aimAt, clearArena, stepPast } from './arena-testkit';
...
function targetInReach(): void {
  ...
  // Point the pointer, not the angle: the sim recomputes aimAngle every tick,
  // and the spear reads it on every frame of the swing. See aimAt.
  aimAt(p.x + cfg().knightSpearRange, p.y);
  g.spawnSkeleton();
