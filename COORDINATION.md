...
| `Wizard` | `feat/wizard-round` | The wizard's round, scope set with Alex: (1) fix what is broken in the bolt, (2) balance the glass cannon, (3) feel and feedback. Focus is out of scope; the talent tree is a retune of the five entries that exist, not a new tier. | `started` | `915d4a1` | `master` `d8f46ad` | `integration/round-7` `0c992b6`; `feat/archer-round` merged into it at `4396004` and touches `game.js` heavily, so this branch's merge meets it — I resolve that, with both diffs open | 09-02 | Worktree `.claude/worktrees/wizard-round`, cut fresh from `master` `d8f46ad`, hooks installed. **Vector 1 done** (`915d4a1`): the ownerless homing-bolt finding was two defects — the bolt walked `crows` alone when choosing what to steer at, *and* its hit loop never mentioned `soldiers`, so the wizard's primary passed straight through the whole garrison. `nearestHostileAmong` in `src/sim/targeting.ts` is the new pure helper; skeletons and soldiers now carry `team: Team.ENEMY` at spawn, which makes the wrapping in `siegeHostiles()` removable (not this round). **Vector 2 in progress:** a time-to-kill harness showed the wizard needed 3.9 s to kill a shieldman the archer kills in 0.33 s, because `bossDamageMult` is boss-only and `balance.md` justified that scope on a claim the garrison outdated — soldiers have 2 and 4 hp, so damage is a quantity there. `wizBoltBodyDamage: 2` fixes it and `docs/balance.md` now records the real scope. Note it also halves his time to kill a scaled crow in waves mode, which is the same defect. **Merge-watch for Merger:** `updateArrows` + `spawnSkeleton`/`spawnSoldier` in `game.js`, and the deleted characterisation test `does NOT home toward a skeleton or a soldier` — a branch that merges over it will look green while re-asserting the bug. **Still to come:** the feel pass and the talent retune. Separately, the archer round's talent-icon work produced **no commits** — `_design/` is gitignored on purpose, so the only record is the design canvas https://claude.ai/code/artifact/aa2f6a93-e370-4855-8fab-f8c2b2ef8846 and the scratch under `_design/talent-feedback/`. Putting those icons in the game means building them as `PixelGrid`s in `src/render`, which is its own branch and has not started. |
