...
| `Aim` | `feat/archer-round` | Release polish: unstick the aim outside the canvas, the post-resize rebalance Stage 6 never landed, tower count vs map area | `finished` | `6926f11` | `master` `d8f46ad` | n/a | 09-02 | Riding on Alex's release branch at his direction rather than a fresh cut, since all four items are for this release. **Aim:** `mouse` was clamped to the canvas in the listener, so past the letterbox edge both axes pinned and the shot froze at the crossing angle -- users reported it as the pointer sticking. Mapping extracted to `pointerToCanvas` in `src/sim/input.ts` (same move as `noteKeyDown`), unclamped there, and `reticleAt` now pins the crosshair where the aim ray leaves the playfield so it stays on the shot line. Verified live through the real listener with synthetic events: 0.000000000 degrees between pointer ray and crosshair ray at four out-of-bounds positions. **Landed since:** `maxPickupsOnMap` 3->8 and `guardSpeed` 74->110, each carrying the 2.62x area figure in its comment. **Not started, and deliberately not on this tip:** scaling the tower count. Alex picked it over deferring, but it is structural rather than a constant -- `towerSites` returns a two-tuple by type -- and it needs a full ten-wave play-through, which has not been scheduled. It comes as its own branch off the round rather than holding these seven commits. `Head` here is the code tip; this ledger commit sits directly above it. |
