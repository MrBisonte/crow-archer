...
| `robinhood-39b` | `feat/talent-trees-r5` | Talent trees: five hero trees, the draft and rite choosers, the eight unlock effects, drawn sigils | `started` | `04f1b43` | current | **NO** | 08-27 | **Branch renamed.** `feat/talent-trees` (head `b6b57ec`) was cut from `feat/char-redesign`; everything below it landed in master via PR #39, so it was re-cut fresh from master `09f42f6` as `feat/talent-trees-r5`. Same content, honest ancestry — do not look for the old name. **Axis re-split, status:** the dead-cell map is landed and measured
