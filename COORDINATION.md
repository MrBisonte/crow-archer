...
| `Wizard` | `feat/wizard-round` | The wizard's round, then the talent system's shape: bolt fixes, glass-cannon balance, feel, a blink talent line, and now the rank ladder plus one uniform tree shape for all five characters. Focus is out of scope. | `started` | `21c4764` | `master` `d8f46ad` | `integration/round-7` `0c992b6`; `feat/archer-round` merged into it at `4396004` and touches `game.js` heavily — I resolve that merge, with both diffs open | 09-02 | Worktree `.claude/worktrees/wizard-round`, cut fresh from `master` `d8f46ad`, hooks installed. **Bolt** (`915d4a1`): the ownerless homing finding was two defects — it walked `crows` alone when choosing what to steer at, and its hit loop never mentioned `soldiers`, so his primary passed through the whole garrison. New pure `nearestHostileAmong`; skeletons and soldiers now carry `team: Team.ENEMY` at spawn, which makes the wrapping in `siegeHostiles()` removable (not this round). **Balance** (`5893601`): a time-to-kill harness showed 3.9 s to kill a shieldman the archer kills in 0.33 s, because `bossDamageMult` is boss-only and `balance.md` justified that scope on a claim the garrison outdated. `wizBoltBodyDamage: 2`. Also halves his time to kill a scaled crow in waves mode — same defect, written down. **Blink line** (`10da552`): LONG STEP -> HELD STEP -> THIRD STEP (**the first tier-III talent in any tree**) -> THUNDERSTEP (a third capstone). New pure `src/sim/blink.ts`. **Ranks:** `RANK_THRESHOLDS` was `[4,10,18]` against a campaign that banks 15, and a ceremony only opens on a boss death — so the most a player can hold at any run-1 ceremony is 8. Rank II *and* rank III were unreachable in a first run; the earliest rite was the second boss of the second run, per character. Now `[2,5,8]`: one boss, one tier. `masteryThroughACampaign` in `talents.test.ts` fails if a threshold moves off a boss. **Merge-watch for Merger:** `updateArrows`, `tryWizardBlink`, `spawnSkeleton`/`spawnSoldier`; the wizard rows of `CHAR_TREES`, `TALENT_LOOK`, `SIGILS`, `TALENTS.STATS`; `RANK_THRESHOLDS`; the deleted characterisation test `does NOT home toward a skeleton or a soldier` (a branch merging over it looks green while re-asserting the bug); and two talents-run tests that pinned a capstone set of two and a draft-first chooser order. **Trees levelled** (`785af22`+): archer, knight, ranger and sapper each gained a tier-II and a tier-III, so every tree is now 2/2/1 and `treesShareOneShape` fails if one drifts. Eight new talents, each wired at every call site including the ones that only draw. **The rite is level** (`21c4764`): DEAD EYE, BULWARK, HOLDFAST and MINEFIELD, one each for archer, knight, ranger and sapper, so every hero offers three. Each is on the tool that hero's other two rites do not touch, which is what THUNDERSTEP already was to the blink. `treesShareOneShape` asserts three, not `>= 2`. **Tier prices levelled** (`a14844d`): the shape test counted talents and passed while the ranger's tree ran 64% dearer than the knight's — tier II is now five points a talent for everyone (FULL TILT loses its third level and keeps its 45% ceiling at 7.5%/level; CHARGE THROUGH and STICKY FAN go 2 → 5), and a second guard says something in a tier must be affordable at the rank that opens it. **Merge-watch, additionally:** `updateBarrageBombs` (two pre-existing bugs fixed in it — the garrison was never in the contact loop, and a stuck bomb kept scanning so a STICKY FAN bomb under a body never ran its fuse out), `damagePlayer`'s shield branch, `damageBoss`/`damageSkeleton`/`damageSoldier` (a shared `netHoldMult`), `releaseArcherDraw`, `holdEnemiesInRadius`, and the new `SAPPER_MINE_ARMED` event. Separately, the archer round's talent-icon work produced **no commits**; `_design/` is gitignored, so the record is the canvas https://claude.ai/code/artifact/aa2f6a93-e370-4855-8fab-f8c2b2ef8846 and the scratch under `_design/talent-feedback/`. That pass continued this session: the knight's, sapper's and ranger's 32x32 icons were redrawn to the archer/wizard bar and the canvas republished; the lessons are in that dir's README, still uncommittable. One thing did reach the repo — `2c95586`: the in-game `shrapnel` sigil carried the same three-path bolt pasted three times, so it drew one arrow and the sheet showed nothing wrong. `src/render/talent-sigils.test.ts` now refuses a repeated path inside a sigil — the one fault in this set that survives being looked at — and pins the sigil key set against the trees in both directions. |
