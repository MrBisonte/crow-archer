...
| `Wizard` | `feat/wizard-round` | The wizard's round, scope set with Alex: (1) fix what is broken in the bolt, (2) balance the glass cannon, (3) feel and feedback. Focus is explicitly out of scope; the talent tree is a retune of the five entries that exist, not a new tier. | `started` | `pending` | `master` `d8f46ad` | `integration/round-7` `0c992b6` (Merger opened it 09-02; this row still needs re-homing under the new header) | 09-02 | Worktree `.claude/worktrees/wizard-round`, cut fresh from `master` `d8f46ad`, hooks installed. **Landed so far:** the homing-bolt finding this ledger carried as ownerless is fixed, and it was two defects rather than one -- the bolt walked `crows` alone when choosing what to steer at, *and* its hit loop never mentioned `soldiers`, so the wizard's primary passed straight through the whole garrison. Both are now covered by tests that were watched failing against the specific reverted line. `nearestHostileAmong` in `src/sim/targeting.ts` is the new pure helper; skeletons and soldiers now carry `team: Team.ENEMY` at spawn, which they always should have -- that omission is why `siegeHostiles()` wraps every body it hands out, and it is now removable (not this round). **Still to come:** the balance pass and the feel pass. Separately, the archer round's talent-icon work produced **no commits** -- `_design/` is gitignored on purpose, so the only record is the design canvas https://claude.ai/code/artifact/aa2f6a93-e370-4855-8fab-f8c2b2ef8846 and the scratch under `_design/talent-feedback/`. Putting those icons in the game means building them as `PixelGrid`s in `src/render`, which is its own branch and has not started. |
