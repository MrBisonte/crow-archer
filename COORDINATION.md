...
| `robinhood-39` | `feat/char-redesign` | Rebuild the archer: profile sprite, walk cycle, live-drawn bow, brace and pierce | `finished`? | `69662b5` | current | yes | 08-26 11:33 | Merged. Status unconfirmed — has not replied. Does it also own `feat/charselect-screen`? |
| *unclaimed* | `feat/charselect-screen` | Rework the char-select screen: per-hero stats, clickable, panel row from canvas | `blocked` | `ddfd0d2` | current | **NO** | 08-26 11:50 | **Owner unknown.** Conflicts with `feat/char-redesign` and `feat/playfield-55x33` in `src/legacy/game.js` and `game.test.ts`. Its author resolves, with both diffs open. |
