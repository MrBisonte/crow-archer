...
| `Icons` (`talent-icons-99`) | `feat/talent-icons` | Talent-tree icons as `PixelGrid` sigils in `src/render` — and the talent-tree completion that rode in with them (a third rite, tier pricing, the new `blink` module) | `started` | `2c5c936` | stacked→wizard | no | 09-02 | **Built on top of `feat/wizard-round`, not `master`** — its history contains every wizard-round commit through `d424479`, then the icon + talent work on top. That is why it **must merge after `feat/wizard-round`**: merging it early drags wizard-round's unfinished `started` work into round 7. It shares wizard-round's actual commit SHAs (not cherry-picks), so the clean path is wizard-round in first, then this adds only its delta — provided **nobody rebases `feat/wizard-round`** out from under it. Bigger than the name: `game.js` (+299), `game.test.ts` (+177), `talents-run.test.ts` (+622), new `src/sim/blink.ts` + `src/sim/targeting.ts`, `src/render/talent-sigils.ts`, `docs/{talents,balance,manual}.md`. Owner `talent-icons-99` still committing; will ping when finished. **The 14 `icons/*` scratch branches + worktrees (all at `2371fcf`) are redundant** — consolidated here — pending prune by the owner. **Check at merge:** it appears to commit `_design/talent-feedback/*.py`, though `_design/` is normally gitignored. |
