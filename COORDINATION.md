...
| `robinhood-39b` | `feat/talent-trees-r5` | Talent trees: five hero trees, the draft and rite choosers, the eight unlock effects, drawn sigils | `started` | `1c768f5` | current | **NO** | 08-27 | **Branch renamed.** `feat/talent-trees` (head `b6b57ec`) was cut from `feat/char-redesign`; everything below it landed in master via PR #39, so it was re-cut fresh from master `09f42f6` as `feat/talent-trees-r5`. Same content, honest ancestry — do not look for the old name. **Axis re-split, status:** the dead-cell map is landed and measured
(`AXIS_HEROES` in `upgrades.ts`, `upgrades-reach.test.ts`) — 13 of 40 cells do nothing
for the hero playing them, and the shop now greys and refuses those rather than selling
them. The MOVE itself is not done and is waiting on the owner: it needs per-hero tier and
price calls plus a decision on already-bought levels. **Planned collision, unchanged:** the axis re-split will move kit axes (arrows/restore/tools/pfRange) OUT of `upgrades.ts` while `fix/upgrade-dead-cells` makes those same axes live per-hero in the same files. Sequence: dead-cells lands first, talents consumes its mappings as seed content. |
