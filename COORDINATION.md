...
|---|---|---|---|---|---|---|---|---|
| `Merger` | `integration/round-5` | Integrate every branch, keep origin clean, open the PR | `started` | `09f42f6` | current | — | 08-27 | Cut from `master` `09f42f6`. Worktree `.claude/worktrees/integration-r5`. Nothing merged yet. |
| `robinhood-39b` | `feat/talent-trees` | Talent trees: per-char tiers gated by mastery, run draft, mid-run capstone rite; wizard pilots | `active` | `b6b57ec` | **STALE** | no | 08-27 | Held out of v0.2.0 deliberately; it is round 5's headline. Cut from `feat/char-redesign` `ead94d7`, which is now on `master` **by content, under a different hash** — so rebase onto `09f42f6`, do not merge, or you will replay 21 commits. Only `src/sim/talents.ts` + `talents.test.ts` (~480 lines) are genuinely new. **Planned collision:** the talent re-split moves kit axes (arrows/restore/tools/pfRange) OUT of `upgrades.ts` while `fix/upgrade-dead-cells` makes those same axes live per-hero in the same files. Sequence: dead-cells lands first, talents consumes its mappings as seed content. |
