...
| `Aim` | `feat/archer-round` | Release polish: unstick the aim outside the canvas, the post-resize rebalance Stage 6 never landed, tower count vs map area | `started` | `pending` | `master` `d8f46ad` | n/a | 09-02 | Riding on Alex's release branch at his direction rather than a fresh cut, since all four items are for this release. **Aim:** `mouse` was clamped to the canvas in the listener, so past the letterbox edge both axes pinned and the shot froze at the crossing angle -- users reported it as the pointer sticking. Mapping extracted to `pointerToCanvas` in `src/sim/input.ts` (same move as `noteKeyDown`), unclamped there, and `reticleAt` now pins the crosshair where the aim ray leaves the playfield so it stays on the shot line. Verified live through the real listener with synthetic events: 0.000000000 degrees between pointer ray and crosshair ray at four out-of-bounds positions. **Still open:** `maxPickupsOnMap` 3->8 and `guardSpeed` 74->110, and scaling the tower count, which is structurally 2 on a map 2.62x the old area. |
