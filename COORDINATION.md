...
| `Balance` | `feat/hero-balance` | Knight, sapper and ranger: the base kits, not the icons. Settled with the owner before any code was written, so the row records the decisions as well as the diff. | `finished` | `895a345` | `master` `d8f46ad` | touches `src/legacy/game.js` CONFIG and `docs/manual.md`; no overlap with the icon branch, which owns `_design/` and nothing under `src/` | 09-02 | **Rebased onto `master`** at Merger's request: it was cut off `feat/wizard-round`, but the only thing in it that needed that branch was one paragraph of `docs/balance.md` naming a talent `master` does not have, and `feat/wizard-round` has gone inactive with two finished branches queued behind it. The code applied to `master` with no conflict outside this ledger. **Knight.** The owner's call was tune the tree, not the kit, with one exception: the charge. It ran at 0.5x his walking speed, which is 75 px/s -- slower than every other hero walking -- for 1.5 s, so it carried 112 px against a 90 px reach. The slowest hero on the roster had a gap-closer that put him barely further forward than he could already hit from, after a windup that rooted him for up to three seconds. It is 1.6x now, 360 px, and the chain goes 1.1x -> 2.0x. Bloodlust is deliberately untouched: `docs/balance.md` argues it is worth least in the fight that is easiest to hold contact through, and the owner agreed. The block stays at one hit on a 10 s timer because the tree is the answer to it and a strong base would leave the talents nothing to buy. The tree itself needed no numeric change: the imbalance was in the kit. **Sapper.** The chain was the fourth build-up on the roster and the only one with nothing on screen -- brace, momentum and bloodlust all report themselves, and his multiplied a boss's damage silently. It is a lane-D chip now, holding the depth the last cascade reached for two seconds, because the whole cascade resolves in five frames and there was otherwise nothing to read. `docs/balance.md` gets the row it never had, and writing it turned up the tree's one real finding: `sapperChainMaxLinks` is depth, not bomb count, so MORE LINKS buys nothing on a stock five-bomb fan and only pays once WIDER FAN has widened it. Ammo and both big buttons are unchanged, per the owner. **Ranger is NOT in this branch** and is deliberately left for a later one: a reload replacing the uncapped click, the net's hold rewritten to actually stop bodies and never bosses, and momentum given something on screen. Those are three new mechanics rather than dials, and the owner chose to land the two cheap heroes first. **Played before it was handed over**, not only tested: the charge and the chain chip were driven in a real run off this worktree. The one thing NOT looked at is the chip's own pixels -- the browser pane kept going hidden, which stops rAF and makes the flight recorder alarm `no-frames`; the live state was confirmed at `peak: 2` instead. **Dependency worth knowing:** the only thing here that needs `feat/wizard-round` is one paragraph of `docs/balance.md` naming WIDER FAN. That paragraph is rewritten to the tree `master` actually has, and it says something sharper there: nothing in the current tree widens the fan, so MORE LINKS pays only on his thrown charges. |
