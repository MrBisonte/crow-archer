...
- **The worktrees live inside the main repo,** at
  `<repo>/.claude/worktrees/<name>`. If a worktree's `.git` file is missing
  (deleted, or its worktree pruned while the session was still running), every
  git command from that directory silently resolves to the main clone,
...
- **There can be more than one clone of this repo on disk**, each with
  separate refs and its own stale `master`. A branch you cannot find is
  probably in another one.
