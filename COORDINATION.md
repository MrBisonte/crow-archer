...
| `Balance` | `feat/hero-balance` | Knight, sapper and ranger: the base kits, not the icons. Settled with the owner before any code was written, so the row records the decisions as well as the diff. | `started` | `(knight done)` | `feat/wizard-round` `d424479` | touches `src/legacy/game.js` CONFIG and `docs/manual.md`; no overlap with the icon branch, which owns `_design/` and nothing under `src/` | 09-02 | **Stacked on `feat/wizard-round`, not on `master`**, for the same reason the icon branch is: `master`'s `talents.ts` has 25 talents and the tree this tunes has 40. Merge after the wizard round. **Knight.** The owner's call was tune the tree, not the kit, with one exception: the charge. It ran at 0.5x his walking speed, which is 75 px/s -- slower than every other hero walking -- for 1.5 s, so it carried 112 px against a 90 px reach. The slowest hero on the roster had a gap-closer that put him barely further forward than he could already hit from, after a windup that rooted him for up to three seconds. It is 1.6x now, 360 px, and the chain goes 1.1x -> 2.0x. Bloodlust is deliberately untouched: `docs/balance.md` argues it is worth least in the fight that is easiest to hold contact through, and the owner agreed. The block stays at one hit on a 10 s timer because TOWER GUARD and BULWARK are the tree's answer to it and a strong base would leave them nothing to buy. The tree itself needed no numeric change -- the pricing guards from `a14844d` still pass and the imbalance was in the kit. |
