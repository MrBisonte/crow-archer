...
| `robinhood-39b` | `feat/talent-trees` | Talent trees: pure model landed (b6b57ec) — per-char tiers gated by mastery, run draft, mid-run capstone rite; wizard pilots | `active` | `b6b57ec` | current | **NO** | 08-27 | Same session as robinhood-39, new branch cut from char-redesign ead94d7. **Planned collision:** the talent re-split will move kit axes (arrows/restore/tools/pfRange) OUT of upgrades.ts while `fix/upgrade-dead-cells` (agent brief issued) makes those same axes live per-hero in the same files. Sequence: dead-cells lands first, talents consumes its mappings as seed content. |
