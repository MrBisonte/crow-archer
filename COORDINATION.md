...
| `Ultimates` | `feat/ultimates` | One ultimate per hero: a long-cooldown skill shot for each of the five, triggered through inputs that already exist -- no new key | `started` | `pending` | `master` `e4feabe` | no | 09-03 | Cut from a freshly fetched `master` `e4feabe` (PR #43, round 7) at Alex's direction, NOT off the current integration branch. **Read this before merging:** round 8 is phase 0 of the native-client remake, so a gameplay branch and a renderer port are live at once; the collision surface is `src/legacy/game.js` -- this branch adds sim state, input handling and HUD chips there, round 8 moves the draw path. Merge order is Merger's call. **Design not settled yet** -- Alex is answering a set of multiple-choice questions on trigger, cost and effect per hero before any code is written; the fixed constraints he has given are: a 30-60 s cooldown, always a skill shot (aimed and missable), and no new keybind. **The ranger's base kit is still deferred** (reload, the net's broken hold, momentum feedback) -- it is not this branch and is still owed. |
