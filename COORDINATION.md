...
| `Towers` | `feat/bastion-tower-count` | Scale the bastion tower count with the grid, the fourth item Alex picked during the release polish | `finished` | `a0ef7dc` | `master` `d8f46ad` | n/a | 09-02 | Cut off `master` at Merger's direction, not off `feat/archer-round`, which is already folded into round 7. **Scaled by height, not area, and that is a deliberate departure from what Alex picked.** The option he chose said "follow the grid area, roughly 5 at 55x33"; area includes width, and `BARRIER_REACH_COLS` on the same file already settled that extra width is open ground for the siege to cross rather than a proportionally larger keep. Scaling by area would have reversed a decision that shipped one round ago. Height is the axis that survives it: the towers stand in one column and spread along it. Result is **4 towers at 55x33** (rows 3, 8, 24, 29), not 5 -- and 4 is the ceiling the geometry allows, not a tuning pick, because `isSpawnZone` clears +/-3 rows of centre and `TOWER_SPAN` needs separation, so a fifth has nowhere symmetric to stand. 21 rows keeps its exact shipped layout, gates stay 4/4 covered at every size measured. `towerSites` now returns `[TowerSite, TowerSite, ...TowerSite[]]`, which keeps the three `[north, south]` destructures typing under `noUncheckedIndexedAccess`. **Not play-tested.** The ten-wave run is the only check that can see whether four towers make the siege too easy; a green suite cannot. Alex has that call. `Head` is the code tip; this ledger commit sits directly above it. |
