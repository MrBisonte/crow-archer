...
(`src/legacy/game.js:2421`). An unkillable boss has no death sequence to run. He does carry a
`BOSS_HP_KEY` row now, but `bossHpFor` answers `Infinity` unless a siege is
running, so in the maze nothing can spend it.
