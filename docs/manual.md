...
- **Primary:** Magic bolts, 1.2 s cooldown, home toward the nearest enemy of any kind, disappear on contact. Worth 2 to a body with health to lose — a spearman falls to one and a shieldman to two — which is the biggest single hit any hero lands on anything short of a boss. See [balance](balance.md#what-a-hit-is-worth-to-a-body)
...
- **Charge (hold Shift):** Winds up in place for up to 3 s, taking no damage while he holds it. Releasing sends him forward at 1.6x his walking speed for 1.5 s -- about 360 px of ground, four spear lengths -- sweeping a 45 degree arc 90 px in front. Anything caught in the arc dies outright; the boss takes 2.6 on an instant release, up to 4 at a full hold, once per charge. That is the charge's own damage figure and it does not scale off the spear's. The invulnerability ends the moment he starts moving. 4 s cooldown
- **Charge chain (tap Shift again):** Within 1.1 s of releasing, and with room left ahead, a second tap commits him harder: the dash goes from 1.6x his walking speed to 2x for the rest of its run, and he lands one whirlwind swing, 60 px, where he is standing. Once per dash. He cannot steer with it — the angle was fixed at release and stays fixed — so the chain buys speed along a line already chosen, not a new one
